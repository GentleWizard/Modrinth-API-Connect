...
def search(query: str, limit: int = 5, offset: int = 0, facets: list | None = None):
    """
    ...
    if status_code != 200:
        ...
        return "No results found" if len(response["hits"]) == 0 else response["hits"]



def get(project_id: str | None = None, slug: str | None = None):
    """
...
def get_multiple(project_ids: list):
    ...
    # params
    params = {"user_ids": json.dumps(project_ids)}

    # make request
    response, status_code = request(
        api_multiple_projects_url, params=params, method="GET"
    )
...
def get_random(count: int):
    ...
    # make request
    response, status_code = request(
        api_random_projects_url, params=params, method="GET"
    )
...
def edit_project(
    ...
    donation_urls: list[str, ...] | None = None,
...
def edit_multiple_projects(project_ids: list, data: dict):
    """
    ...
    # make request
    response, status_code = request(
        api_modify_multiple_projects_url, params=params, method="Patch", data=data
    )
...
def edit_project_icon(project_id: str, icon: str, slug: str | None = None):
    """
    ...
    # params
    files = {"icon": image_data}

    # make request
    response, status_code = request(
        api_modify_project_icon_url, files=files, method="Patch"
    )
...
def edit_gallery_image(
    ...
    # make request
    response, status_code = request(
        api_modify_gallery_image_url, data=data, method="Patch"
    )
