...
from .utils.SortData import dict_result, list_result
...
class Team:
    ...
    def get_Project_Members(self, id: str = None, slug: str = None, data = None, async_: bool = False):
        
        
        # set API endpoint
        api_project_members_url = f'{self.base_url}/team/{id or slug}/members'
        ...
        if async_:
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(Request, api_project_members_url)
                response, status_code = future.result()
        else:
            response, status_code = Request(url=api_project_members_url)
        ...
        if status_code != 200:
            ...
            # return data
            return list_result(None, data, response)